...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("SELECT receipt_number FROM juapong_payments WHERE receipt_number = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // Insert only the specified fields into juapong_payments
        $stmt = $conn->prepare("INSERT INTO juapong_payments (payment_id, account_number, business_name, amount, payment_date, receipt_number) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssdds", $payment_id, $account_number, $business_name, $amount, $payment_date, $receipt_number);
        if ($stmt->execute()) {
            $message = "Payment added successfully!";
            // Update juapong_businesses with amount
            $stmt = $conn->prepare("UPDATE juapong_businesses SET previous_payment = previous_payment + ?, arrears = arrears - ? WHERE account_number = ?");
...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $query = "SELECT account_number, business_name, old_fee, previous_payment, arrears, current_fee, amount_payable FROM juapong_businesses WHERE account_number LIKE ? OR business_name LIKE ? LIMIT 1";
...
<head>
    ...
    <title>Quickbil - Add Juapong Payment</title>
    ...
    <div class="flex">
        <div class="bg-gray-900 w-72 p-6 fixed h-full shadow-2xl border-r border-gray-800">
            ...
            <nav class="space-y-8">
                ...
                <a href="mepe_payment_add.php" class="flex items-center py-3 px-4 hover:bg-gray-800 rounded-xl transition duration-300 text-lg font-semibold"><span class="mr-3">💰</span> Mepe Payments</a>
                <a href="juapong_payment_add.php" class="flex items-center py-3 px-4 bg-blue-700 rounded-xl hover:bg-blue-800 transition duration-300 text-lg font-semibold shadow-md"><span class="mr-3">💰</span> Juapong Payments</a>
        ...
        <div class="flex-1 p-12 ml-72">
            <h1 class="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500 mb-6 drop-shadow-lg">Add Juapong Payment</h1>
